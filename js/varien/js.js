...
jQuery(document).ready(function () {
    ...
    if (responsiveMode) {
        ...
        jQuery(window).on('scroll', function () {
            clearTimeout(id2);
            id2 = setTimeout(closeOnScroll, 100); // si el usuari va molt rÃ pid se li pot tancar el hover. PerÃ² aixÃ­ evitem tantes crides
        });
        ...
        timeout = setTimeout(() => {
            ...
            jQuery('#pagination-container').pagination({
                ...
                callback: async function (data, pagination) {
                    //const html = data.map(item => `<div>${item.name} - ${item.age} aÃ±os</div>`).join('');
    ...
    jQuery('.sales-order-history').on('keydown', '#searchInput', function (e) {
        if (e.key === 'Enter' || e.keyCode === 13) {
            e.preventDefault(); // Evita que se envÃ­e un formulario si lo hay
            jQuery('#searchButton').trigger('click');
        }
    });

    Validation.add('validate-nif', 'El NIF/CIF/NIE no es vÃ¡lido', function (value) {
    ...
    jQuery("#enviar_factura").on('click', function (e) {
        ...
        // AsegÃºrate de usar el ID correcto del formulario
    ...
    /*jQuery(document).on('click', '.invoice-link', function (e) {
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                ...
                // Mostrar el modal (ajusta segÃºn cÃ³mo uses el modal)
                // Extrae el JSON desde un elemento del HTML si estÃ¡ en el bloque
...
function validarNifCifNie(valor) {
    ...
    // Validar NIF (8 nÃºmeros + 1 letra)
    ...
    // Validar NIE (X, Y, Z + 7 nÃºmeros + 1 letra)
    ...
    // Validar CIF (1 letra + 7 nÃºmeros + 1 dÃ­gito o letra)
    if (/^[ABCDEFGHJNPQRSUVW][0-9]{7}[0-9A-J]$/.test(valor)) {
        ...
        if (/[ABEH]/.test(letraInicial)) {
            ...
            // Puede ser un nÃºmero o una letra
            return control == unidad || control === letrasControl.charAt(unidad);
        }
    }
    return false; // No coincide con ningÃºn formato
}
...
function initializeBillingRegionUpdater(regionsJson) {
    if (typeof RegionUpdater !== 'undefined') {
        ...
        if (countrySelect && countrySelect.val()) {
            ...
            }, 100); // pequeÃ±o retraso para dejar actuar a RegionUpdater
        }
    } else {
        console.error("RegionUpdater no estÃ¡ definido");
    }
}
...
function change_required_region(type) {
    ...
    if (RegionIsASelect) {	// Si la regiÃ³n es un select, entonces el campo es obligatorio
